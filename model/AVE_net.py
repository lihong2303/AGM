...
class GradMod(nn.Module):
    def __init__(self,cfgs):
        ...
        self.cfgs = cfgs
        self.mode = cfgs.mode
        self.net = psp_net(128,512,128,29)
    ...
    def forward(self,audio,visual):
        total_out,cross_attn = self.net(audio,visual,self.cfgs.threshold,pad_audio = False,pad_visual=False)
        self.net.eval()
        pad_visual_out,_ = self.net(audio,visual,self.cfgs.threshold,pad_audio = False,pad_visual=True)
        pad_audio_out,_ = self.net(audio,visual,self.cfgs.threshold,pad_audio = True,pad_visual=False)
        zero_padding_out,_ = self.net(audio,visual,self.cfgs.threshold,pad_audio = True,pad_visual=True)
        if self.mode=="train":
            self.net.train()
        ...
        return m_a,m_v,m_a+m_v,cross_attn,c
...
class SelfAttention(nn.Module):
    ...
    def forward(self,audio_feature):
        ...
        a_seg_rel = torch.bmm(phi_a,theta_a.permute(0,2,1))
...
class AVGA(nn.Module):
    ...
    def forward(self,audio,video):
        V_DIM = video.size(-1)
        v_t = video.view(video.size(0)*video.size(1),-1,V_DIM)
        V = v_t
        ...
        c_t = torch.bmm(alpha_t,V).view(-1,V_DIM) 
        video_t = c_t.view(video.size(0),-1,V_DIM)

        return video_t

class LSTM_A_V(nn.Module):
    def __init__(self,a_dim,v_dim,hidden_dim=128,seq_num=10):
    ...
    def init_hidden(self,a_fea,v_fea):
        ...
        hidden_a = (torch.zeros(2,bs,a_dim).to(a_fea.device),torch.zeros(2,bs,a_dim).to(a_fea.device))
        hidden_v = (torch.zeros(2,bs,a_dim).to(a_fea.device),torch.zeros(2,bs,a_dim).to(a_fea.device))
        return hidden_a,hidden_v

    def forward(self,a_fea,v_fea):
        hidden_a,hidden_v = self.init_hidden(a_fea,v_fea)
        self.lstm_audio.train()
        self.lstm_video.train()
...
class PSP(nn.Module):
    """
    Positive Sample Propagation module
    """
    def __init__(self,a_dim=256,v_dim=256,hidden_dim=256,out_dim=256) -> None:
    ...
    def forward(self,a_fea,v_fea,thr_val):
        v_branch1 = self.dropout(self.activation(self.v_L1(v_fea)))
        ...
        beta_va = torch.bmm(v_branch2,a_branch1.permute(0,2,1))
        beta_va /= torch.sqrt(torch.FloatTensor([v_branch2.shape[2]]).to(a_fea.device))

        beta_va = F.relu(beta_va)
        beta_av = beta_va.permute(0,2,1)

        sum_v_to_a = torch.sum(beta_va,dim=-1,keepdim=True)
        beta_va = beta_va / (sum_v_to_a + 1e-8)
        gamma_va = (beta_va > thr_val).float() * beta_va
        sum_v_to_a = torch.sum(gamma_va, dim=-1, keepdim=True)
        ...
        v_psp = self.dropout(self.activation(self.v_fc(v_psp)))
        a_psp = self.dropout(self.activation(self.a_fc(a_psp)))
...
class AVSimilarity(nn.Module):
    """
    function to compute audio-visual simility.
    """
    def __init__(self):
    ...
    def forward(self,v_fea,a_fea):
        ...
        cos_simm = torch.sum(torch.mul(v_fea,a_fea),dim=-1)
        return cos_simm
...
class psp_net(nn.Module):
    ...
    def __init__(self,a_dim=128,v_dim=512,hidden_dim=128,category_num = 29):
        ...
        self.mode = "classify"
        self.compute_layer_conductance = False
    ...
    def forward(self,audio,video,thr_val = 0.099,pad_audio = False,pad_visual=False):
        if pad_audio:
        ...
        video_t = self.attention(fa_fea,video)
        video_t = self.fv(video_t)
        lstm_audio,lstm_video = self.lstm_a_v(fa_fea,video_t)
        fusion,final_v_fea,final_a_fea = self.psp(lstm_audio,lstm_video,thr_val=thr_val)
        cross_att = self.av_simm(final_v_fea,final_a_fea)

        encoded_out = self.relu(self.L1(fusion))
        out = self.L2(encoded_out) # [bs,10,29]
        if self.mode == "feature":
            return encoded_out.reshape(-1,64),out,cross_att
        if self.compute_layer_conductance:
            out = out.reshape(-1,29)
            return out
        return out,cross_att